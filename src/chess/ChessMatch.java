...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPostion sourcePosition, ChessPostion targetPosition) {
		...
		if (movedPiece instanceof Pawn) {
			if (movedPiece.getColor() == Color.WHITE && target.getLinha() == 0|| movedPiece.getColor() == Color.BLACK && target.getLinha() == 7) {
				promoved = (ChessPiece)board.piece(target);
				promoved = replacePromoterPiece("Q");
	...
	public ChessPiece replacePromoterPiece(String type) {
	    ...
	    Position pos = promoved.getChessPosition().toPosition();
	    Piece p = board.removePiece(pos);
	    piecesOnTheBoard.remove(p);

	    ChessPiece newPiece = newPiece(type, promoved.getColor());
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		// En passant
		if (p instanceof Pawn) {
			if (source.getColuna() != target.getColuna() && capturedPiece == enPassant) {
	...
	private ChessPiece king(Color color) {
		...
		throw new IllegalStateException("O rei da cor " + color + " nao foi encontrado no tabuleiro");
	}
